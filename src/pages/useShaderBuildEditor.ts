...
import { defaultPatterns } from '../patterns/pages/defaultPatterns';
import { palettePresets } from '../patterns/palettePresets';
import type {
  ShaderPattern,
  EditorParameter,
  GradientPalette,
...
const linkProgram = (gl: WebGL2RenderingContext, vs: WebGLShader, fs: WebGLShader): WebGLProgram | null => {
...
export function useShaderBuildEditor() {
  // --- State ---
  const patterns = defaultPatterns;
  // --- URL Hash Shared State Loader (Synchronous initialization) ---
  ...
  const [selectedPattern, setSelectedPattern] = useState<ShaderPattern>(() => {
    return hashState ? hashState.targetPattern : defaultPatterns[0];
  });
  ...
  const [docName, setDocName] = useState<string>(() => {
    return hashState ? hashState.state.name : defaultPatterns[0].name;
  });
  const [parameters, setParameters] = useState<EditorParameter[]>(() => {
    const pattern = hashState ? hashState.targetPattern : defaultPatterns[0];
    if (hashState && hashState.state.params) {
      return pattern.defaultParameters.map(p => {
        const found = hashState.state.params.find(sp => sp.k === p.key);
        return found ? { ...p, value: found.v } : p;
      });
    }
    return pattern.defaultParameters;
  });
  const [palette, setPalette] = useState<GradientPalette>(() => {
    const pattern = hashState ? hashState.targetPattern : defaultPatterns[0];
    if (hashState && hashState.state.stops) {
      return {
        id: pattern.defaultPalette.id,
        name: pattern.defaultPalette.name,
        stops: hashState.state.stops.map((s, i) => ({ id: `stop_${i}`, color: s.c, position: s.p })),
        interpolation: pattern.defaultPalette.interpolation
      };
    }
    return pattern.defaultPalette;
  });
  const [codeSource, setCodeSource] = useState<string>(() => {
    const pattern = hashState ? hashState.targetPattern : defaultPatterns[0];
    return pattern.renderEngine === 'css'
      ? (pattern.cssSource ?? '')
  ...
  const lastValidShaderSourceRef = useRef<string>('');
  if (lastValidShaderSourceRef.current === '') {
    const pattern = hashState ? hashState.targetPattern : defaultPatterns[0];
    lastValidShaderSourceRef.current = pattern.shaderSource ?? '';
  }
  ...
  const [modalState, setModalState] = useState({
    ...
    settingsOpen: false,
    setOpen: false,
    kind: null as 'png' | 'video' | 'gif' | null,
    exportType: 'code' as 'code' | 'image' | 'react' | 'css-export'
  });
  ...
  const [viewConfig, setViewConfig] = useState(() => {
    ...
    return {
      comparePercent: 50,
      aspectRatio: '16:9',
      activeStopId: pattern.defaultPalette.stops[0]?.id || null,
      preview: {
        width: 1920,
        height: 1080,
        fitMode: 'contain' as 'contain' | 'cover' | 'actual',
        playing: true,
        time: 0,
        loopLength: 10.0
      }
  ...
  const [setModalSettings, setSetModalSettings] = useState({
    setCount: 6,
    setRes: '1080'
  });

  // Local bindings to avoid touching existing logic
  const recordingVideo = exportStatus.recordingVideo;
  const setRecordingVideo = (val: boolean) => setExportStatus(prev => ({ ...prev, recordingVideo: val }));
  const recordingGIF = exportStatus.recordingGIF;
  const setRecordingGIF = (val: boolean) => setExportStatus(prev => ({ ...prev, recordingGIF: val }));
  const recordProgress = exportStatus.progress;
  const setRecordProgress = (val: number) => setExportStatus(prev => ({ ...prev, progress: val }));
  const exportingTitle = exportStatus.title;
  const setExportingTitle = (val: string) => setExportStatus(prev => ({ ...prev, title: val }));
  const exportingDetail = exportStatus.detail;
  const setExportingDetail = (val: string) => setExportStatus(prev => ({ ...prev, detail: val }));

  const imgRes = exportConfig.imgRes;
  const setImgRes = (val: string) => setExportConfig(prev => ({ ...prev, imgRes: val }));
  const vidRes = exportConfig.vidRes;
  const setVidRes = (val: string) => setExportConfig(prev => ({ ...prev, vidRes: val }));
  const vidFps = exportConfig.vidFps;
  const setVidFps = (val: string) => setExportConfig(prev => ({ ...prev, vidFps: val }));
  const vidLen = exportConfig.vidLen;
  const setVidLen = (val: string) => setExportConfig(prev => ({ ...prev, vidLen: val }));
  const gifW = exportConfig.gifW;
  const setGifW = (val: string) => setExportConfig(prev => ({ ...prev, gifW: val }));
  const gifFps = exportConfig.gifFps;
  const setGifFps = (val: string) => setExportConfig(prev => ({ ...prev, gifFps: val }));
  const gifDither = exportConfig.gifDither;
  const setGifDither = (val: boolean) => setExportConfig(prev => ({ ...prev, gifDither: val }));
  const gifLoop = exportConfig.gifLoop;
  const setGifLoop = (val: boolean) => setExportConfig(prev => ({ ...prev, gifLoop: val }));

  const exportModalOpen = modalState.exportOpen;
  const setExportModalOpen = (val: boolean) => setModalState(prev => ({ ...prev, exportOpen: val }));
  const settingsModalOpen = modalState.settingsOpen;
  const setSettingsModalOpen = (val: boolean) => setModalState(prev => ({ ...prev, settingsOpen: val }));
  const setModalOpen = modalState.setOpen;
  const setSetModalOpen = (val: boolean) => setModalState(prev => ({ ...prev, setOpen: val }));
  const exportModalKind = modalState.kind;
  const setExportModalKind = (val: 'png' | 'video' | 'gif' | null) => setModalState(prev => ({ ...prev, kind: val }));
  const exportType = modalState.exportType;
  const setExportType = (val: 'code' | 'image' | 'react' | 'css-export') => setModalState(prev => ({ ...prev, exportType: val }));

  const comparePercent = viewConfig.comparePercent;
  const setComparePercent = (val: number) => setViewConfig(prev => ({ ...prev, comparePercent: val }));
  const aspectRatio = viewConfig.aspectRatio;
  const setAspectRatio = (val: string) => setViewConfig(prev => ({ ...prev, aspectRatio: val }));
  const activeStopId = viewConfig.activeStopId;
  const setActiveStopId = (val: string | null) => setViewConfig(prev => ({ ...prev, activeStopId: val }));
  const preview = viewConfig.preview;
  const setPreview = (val: React.SetStateAction<PreviewSettings>) => {
    setViewConfig(prev => {
      const nextPreview = typeof val === 'function' ? val(prev.preview) : val;
      return { ...prev, preview: nextPreview };
    });
  };

  const setCount = setModalSettings.setCount;
  const setSetCount = (val: number) => setSetModalSettings(prev => ({ ...prev, setCount: val }));
  const setRes = setModalSettings.setRes;
  const setSetRes = (val: string) => setSetModalSettings(prev => ({ ...prev, setRes: val }));
  ...
  const handleSelectPattern = (pattern: ShaderPattern) => {
    ...
    if (saved) {
      ...
      lastValidShaderSourceRef.current = codeToLoad;
  ...
  // --- URL Hash Shared State Loader (Side Effects) ---
  useEffect(() => {
    if (hashState) {
      startTimeRef.current = Date.now();
      timeRef.current = 0;
      showToast(`Loaded shared pattern: ${hashState.state.name}`);
    }
  }, [hashState]);
  ...
  useEffect(() => {
    ...
    if (isWebGL) {
      ...
      try {
        ...
        fragmentShader = compileShader(gl, codeSource, gl.FRAGMENT_SHADER);

        if (vertexShader && fragmentShader) {
          localProgram = linkProgram(gl, vertexShader, fragmentShader);
          if (localProgram) {
            programRef.current = localProgram;
            setCompileError(null);
            lastValidShaderSourceRef.current = codeSource;
          }
        }
      } catch (e: unknown) {
        ...
        if (lastValidShaderSourceRef.current) {
          try {
            const fallbackVS = compileShader(gl, vsSource, gl.VERTEX_SHADER);
            const fallbackFS = compileShader(gl, lastValidShaderSourceRef.current, gl.FRAGMENT_SHADER);
            if (fallbackVS && fallbackFS) {
              localProgram = linkProgram(gl, fallbackVS, fallbackFS);
              if (localProgram) {
                programRef.current = localProgram;
              }
    ...
    const tick = () => {
      ...
      if (isWebGL && canvas && currentGl && activeProgram && vao) {
        ...
        const sortedStops = palette.stops.toSorted((a, b) => a.position - b.position);
  ...
  }, [exportModalKind, selectedPattern]);
    ...
    const drawSetTiles = async () => {
      ...
      if (selectedPattern.renderEngine !== 'css' && gl && program && vao) {
        ...
        const renderTile = async (i: number) => {
          if (i >= seeds.length || !active) return;
  ...
  }, [setModalOpen, setCount, selectedPattern, palette, parameters, aspectRatio, seeds]);
  ...
  const handleDownloadSet = async () => {
    ...
    const renderNext = async (i: number) => {
      if (i >= seedsList.length) return;
      if (!recordingVideo && !recordingGIF) return;
  ...
  const handleAddStop = () => {
    ...
    const sorted = palette.stops.toSorted((a, b) => a.position - b.position);
    let newPos = 0.5;
    if (sorted.length >= 2) {
      // Find widest gap
      let maxGap = 0;
  ...
  const handleStopMouseDown = (_e: MouseEvent<HTMLElement>, id: string) => {
    setActiveStopId(id);
  ...
  const handleStopTouchStart = (_e: TouchEvent<HTMLElement>, id: string) => {
    setActiveStopId(id);
  ...
  const handleStopKeyDown = (e: KeyboardEvent<HTMLElement>, id: string) => {
